...
- **Configurable destinations**: URL templates with a `{term}` placeholder, each with its own label. Add and edit them on the settings page.
...
1. **Add a destination** — open **Settings** from the panel, then press **+ Add** under **Destinations**. The URL template is the trick: run a search on the site, copy the resulting URL, and replace your search words with `{term}`.
...
Quick-search slots map to your first five destinations, in the order they appear in the destinations list.
