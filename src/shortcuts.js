...
(function () {
  ...
  //   { shortcut, error }          — bound, but the panel will never see it
  //   { shortcut: "", combo: null } — the slot is simply unbound
  //
  // The error cases are worth surfacing: a binding that can't be intercepted
  // still works on a page (the background worker handles it against the
  // selection), it just can't search what you typed into the box.
  function describeBinding(shortcut) {
    if (!shortcut) return { shortcut: "", combo: null, error: "" };
    const parsed = parseShortcut(shortcut);
    const key = canonicalKey(parsed && parsed.key);
    const fail = (error) => ({ shortcut, combo: null, error });

    if (!key) return fail("Context can't tell which key this is");
    if (key.startsWith("media")) {
      return fail("Chrome doesn't deliver media keys to extension windows");
    }
    const code = codeFor(key);
    if (!code) return fail(`Context doesn't recognise the "${parsed.key}" key`);
    ...
    if (bare && !/^f([1-9]|1[0-2])$/.test(key)) {
      return fail("a shortcut with no modifier would fire while you type");
    }
    return {
      shortcut,
      error: "",
